...
**Only live runners count as supply**: an instance running longer than `reconciler-boot-grace-seconds` (default 300) counts toward supply *only* if its runner is `online` in GitHub's runner list. A runner that connects but never registers — a failed bootstrap, or GitHub dropping the session as in the 2026-08-06 Actions outage — can never be handed a job, and counting it stalled the backlog for the full 60-minute watchdog timeout. Such instances are deliberately **not** terminated by the reconciler, only discounted: GitHub briefly misreporting a busy runner as offline would otherwise kill a job mid-run, and the watchdog reclaims the instance anyway. `reconciler-max-top-ups` (default 3, tracked in the `topUps` attribute on the row) caps replacements per job so a systemic outage cannot saturate `max-concurrent-runners` with instances that will never come online.
